...
* **Tutoriel sur WKND Events** : si le développement d’applications monopage (SPA) dans AEM vous intéresse particulièrement, consultez notre [tutoriel WKND Events](https://experienceleague.adobe.com/docs/experience-manager-learn/sites/spa-editor/spa-editor-framework-feature-video-use.html?lang=fr) dédié.
* **Commencez par vous-même !** - Vous pouvez facilement télécharger l’archétype de projet [actuel](https://github.com/adobe/aem-project-archetype) disponible sur GitHub et créer votre premier projet vous-même.
...
* **[all](https://github.com/adobe/aem-project-archetype/tree/develop/src/main/archetype/all)** est un module de contenu unique qui incorpore tous les modules compilés (lots et modules de contenu), y compris les dépendances des fournisseurs.
...
![Organisation du module de contenu](/help/assets/content-package-organization.png)

Les modules de l’archétype AEM représentés dans Maven sont déployés vers AEM en tant que modules de contenu représentant l’application, le contenu et les lots OSGi nécessaires.
...
La section `<modules>` du fichier POM parent définit les modules que le projet va créer. Par défaut, le projet crée [les modules standard précédemment définis](#what-you-get). Vous pouvez toujours ajouter d’autres modules à mesure que le projet évolue.
...
L’une des dépendances clés est le fichier Jar d’API Java [AEM.](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/developing/aem-as-a-cloud-service-sdk.html?lang=fr) Cela inclut toutes les API d’AEM avec une seule entrée de dépendance pour la version d’AEM.
...
L’archétype utilise bien sûr les [composants principaux](/help/introduction.md). Par conséquent, afin d’exploiter les composants principaux dans tous les déploiements, il est recommandé de les inclure dans le projet Maven.
