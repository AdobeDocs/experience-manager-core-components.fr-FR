...
En tant que créateur ou créatrice, vous remarquerez que les composants principaux présentent quelques avantages. En voici un aperçu :
...
La configuration des composants de base était du ressort d’un membre de l’équipe de développement. Toutefois, avec les composants principaux, un créateur ou une créatrice de modèles peut maintenant configurer plusieurs fonctionnalités via l’éditeur de modèles.
...
Les composants principaux pouvant être préconfigurés par les créateurs ou créatrices de modèles pour définir les options autorisées dans le cadre d’un modèle, puis configurés par le créateur ou la créatrice de la page pour définir le contenu réel de la page, chaque composant peut avoir des options dans deux boîtes de dialogue différentes.
...
## Ressources de développement {#developer-resources}

Pour obtenir des informations techniques sur les composants principaux, voir la documentation [Développement des composants principaux](/help/developing/overview.md).
